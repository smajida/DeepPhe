...
public class SizeTester {
   ...
   public void testSingleMention() {
      final List<Size> cancerSizes = SizeFinder.getSizes( SENTENCE_4 );
      assertEquals( "Expect one Cancer Size in " + SENTENCE_4, 1, cancerSizes.size() );
      final List<Measurement> measurements = new ArrayList<>( cancerSizes.get( 0 ).getMeasurements() );
      assertEquals( "Expect one first measurement in " + SENTENCE_4, 1, measurements.size() );
      testMeasurement( SENTENCE_4, measurements.get( 0 ), 0.9, DimensionUnit.CENTIMETER );
   }

   static private void testMeasurement( final String sentence, final Measurement measurement,
                                        final double expectedValue, final DimensionUnit expectedUnit ) {
      assertEquals( expectedValue, measurement.getValue(), TOLERANCE );
      assertEquals( "Expect " + expectedUnit.getTitle() + " unit in " + sentence,
            expectedUnit.getTitle(), measurement.getUnit() );
   }
