   ...
   static public void addTnms( final JCas jcas, final AnnotationFS lookupWindow,
      ...
      for ( Tnm tnm : tnms ) {
         ...
         if ( tnmPrefix != null ) {
            ...
            TnmPhenotypeFactory.getInstance().createPhenotype( jcas, windowStartOffset, tnm, neoplasms, Collections
                  .singletonList( diagnosticTest ) );
         } else {
            TnmPhenotypeFactory.getInstance().createPhenotype( jcas, windowStartOffset, tnm, neoplasms );
         }
      }
   }
