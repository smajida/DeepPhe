...
public class SentenceDetectorAnnotator extends CleartkAnnotator<String> {
   ...
   public void process( JCas jcas ) throws AnalysisEngineProcessException {
      ...
      if ( featConfig == FEAT_CONFIG.LINE_POS || featConfig == FEAT_CONFIG.CHAR_POS ||
           featConfig == FEAT_CONFIG.CHAR_SHAPE_POS ) {
      ...
      for ( Segment seg : JCasUtil.select( jcas, Segment.class ) ) {
         ...
         for ( int ind = 0; ind < segText.length(); ind++ ) {
            ...
            if ( featConfig == FEAT_CONFIG.CHAR || featConfig == FEAT_CONFIG.CHAR_POS ||
                 featConfig == FEAT_CONFIG.CHAR_SHAPE || featConfig == FEAT_CONFIG.CHAR_SHAPE_POS ) {
               ...
               if ( !prevOutcome.equals( "O" ) && Character.isLetterOrDigit( curChar ) ) {
                  ...
                  if ( outcome.equals( "B" ) ) {
                     ...
                     while ( endInd > startInd &&
                             Character.isWhitespace( segText.charAt( endInd - seg.getBegin() - 1 ) ) ) {
                        endInd--;
                     }
      ...
      for ( Segment seg : JCasUtil.select( jcas, Segment.class ) ) {
         ...
         for ( String line : lines ) {
            if ( line.contains( "[**" ) || line.contains( "**]" ) ) {
               continue; // mimic PHI-replacement artificially extends lines
            }
   ...
   private static String getPrevToken( String segText, int ind ) {
      ...
      int startInd = endInd;
      while ( startInd > 0 && !Character.isWhitespace( segText.charAt( startInd ) ) &&
              !Character.isWhitespace( segText.charAt( startInd - 1 ) ) ) {
         startInd--;
      }
   ...
   static CharacterCategoryPatternFunction<Annotation> shapeFun
         = new CharacterCategoryPatternFunction<>( PatternType.REPEATS_AS_KLEENE_PLUS );
   ...
   private Collection<? extends Feature> getTokenFeatures( String prevToken, String nextToken, String prefix ) {
      ...
      feats.add( new Feature(
            prefix + "cap", nextToken.length() > 0 && Character.isUpperCase( nextToken.charAt( 0 ) ) ) );

      // shape features for word identity
      if ( featConfig == FEAT_CONFIG.CHAR_SHAPE_POS || featConfig == FEAT_CONFIG.CHAR_SHAPE ||
           featConfig == FEAT_CONFIG.SHAPE ) {
      ...
      feats.add( new Feature( "LeftWordRightCap",
            prevToken + "_" + (nextToken.length() > 0 && Character.isUpperCase( nextToken.charAt( 0 ) )) ) );
   ...
   public List<Feature> getPositionFeatures( char curChar, int ind, String segText, String nextToken ) {
      ...
      if ( curChar == '\n' && ind > 0 ) {
         ...
         for ( int intLens = thisLineLength; intLens < thisLineLength + 1 + nextToken.length(); intLens++ ) {
            if ( !endCounts.containsKey( intLens ) ) {
               continue;
            }
