...
public class PatientCollectionReader extends CollectionReader_ImplBase {
	...
	/**
	 * Initialize shell patient objects. We don't populate patients completely to 
	 * support a high number of patients in the future. This way patient init is 
	 * done on a per patient basis during CAS initialization.
	 * 
	 */
	@Override
	...
	public boolean hasNext() throws IOException, CollectionException {
		return patients.iterator().hasNext();
	}
	
	@Override
	public void getNext(CAS aCAS) throws IOException, CollectionException {
		progressIndex++;
		Patient p = patients.iterator().next();
		...
		try {
			...
			String s = new String(serializedPatient,Charset.forName("UTF-8"));
	...
	/**
	 * Loads the patient with actual data. We do this separately than creation of 
	 * shell patient objects because we want to support a high number of patients in the future.
	 * 
	 * @param p
	 * @throws IOException 
	 * @throws ClassNotFoundException 
	 */
	private void loadPatient(File inputDirectory, Patient p) throws IOException, ClassNotFoundException {
		...
		for(File f:patientDir.listFiles()){
			if(!f.isDirectory()){
				 ...
				 FileInputStream inputFileStream = new FileInputStream(f);
			      Encounter encounter = (Encounter)SerializationUtils.deserialize(inputFileStream);
	...
	public Progress[] getProgress() {
		return new Progress[]{new ProgressImpl(progressIndex, patients.size(),Progress.ENTITIES)};
	}
	@Override
